...
export default {
  ...
  mounted() {
    ...
    this.$watch(watcher, () => {
      const fullSchema = schemaUtils.prepareFullSchema(this.resolvedSchema, this.flattenObject(this.modelRoot), this.fullOptions)
      // kinda hackish but prevents triggering large rendering chains when nothing meaningful changes
      if (!deepEqual(fullSchema, this.fullSchema)) this.fullSchema = fullSchema
    }, {
      immediate: true,
    ...
    input(value, initial = false, fastForward = true) {
      ...
      if (value === null || value === undefined) {
        ...
        if (!deepEqual(value, this.modelValue)) {
          ...
          if (fastForward) {
            this.fastForwardEvent('input-child', { key: this.fullKey, value: value })
            this.fastForwardEvent('input-schema', { key: this.fullKey, value: this.fullSchema })
          }
          // this.$emit('input', value)
          this.value[this.fullKey] = value
