  ...
  methods: {
      ...
      if (this.display === 'expansion-panels') {
        ...
        return [h(resolveComponent('v-stepper-window-item'),
          ...
          () => [h(resolveComponent('v-card'), { tile: true, flat: true }, () => [
            ...
            isLast ? null : h(resolveComponent('v-card-actions'), { class: { 'px-0': true } }, () => [h(resolveComponent('v-btn'), {
              color: 'primary',
              onClick: () => {
    ...
    renderObjectContainer() {
      ...
      if (this.display === 'tabs' && sectionsChildren.length) {
        ...
        sectionsChildren = [h('div', { style: 'width: 100%;' }, [h(resolveComponent('v-tabs'), { modelValue: this.currentTab, ...props }, sectionsChildren), h(resolveComponent('v-window'), { modelValue: this.currentTab, 'onUpdate:modelValue': value => { this.currentTab = value } }, windowItems)])]
      }
