    ...
    renderDateProp() {
      ...
      if (this.fullSchema.format === 'date') {
        ...
        child = h(VDatePicker, {
          ...
          modelValue: this.valueDate(this.modelValue),
        ...
        const tabs = [
          h(resolveComponent('v-tab'), { value: 'tab-date', eager: true }, [h(resolveComponent('v-icon'), [this.fullOptions.icons.calendar])]),
          h(resolveComponent('v-tab'), { value: 'tab-time', disabled: !this.dateProp.parts[0], eager: true }, [h(resolveComponent('v-icon'), [this.fullOptions.icons.clock])])
        ]
        const windowItems = [h(resolveComponent('v-window-item'), { value: 'tab-date' }, [h(resolveComponent('v-date-picker'), {
          ...this.fullOptions.datePickerProps,
          locale: this.fullOptions.locale,
          modelValue: this.valueDate(this.dateProp.parts[0]),
          'onUpdate:modelValue': value => {
            if (!(value instanceof Date)) return
        ...
        child = [h(resolveComponent('v-tabs'), {
          props: { grow: true, 'align-tabs': 'center', modelValue: this.dateProp.tab },
